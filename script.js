...
function primerRender(data) {
  ...
  for (let i in items) {
    ...
    ajuste = String(info["price"]["amount"]).split(".")[1]
      ? String(info["price"]["amount"])
        .split(".")[0]
        .split(/(?=(?:...)*$)/) +
        "." +
        String(info["price"]["amount"]).split(".")[1]
      : String(info["price"]["amount"])
        .split(".")[0]
        .split(/(?=(?:...)*$)/);
...
function renderProd(datosProd) {
  ...
  estado.innerHTML =
    datosProd["condition"] + " | " + datosProd["sold_quantity"] + " vendidos";
  ...
  ajuste = String(datosProd["price"]["amount"]).split(".")[1]
    ? String(datosProd["price"]["amount"])
      .split(".")[0]
      .split(/(?=(?:...)*$)/) +
      "." +
      String(datosProd["price"]["amount"]).split(".")[1]
    : String(datosProd["price"]["amount"])
      .split(".")[0]
      .split(/(?=(?:...)*$)/);
  ...
  let srt = favoritos.includes(datosProd)
...
function renderFavoritos() {
  ...
  for (let i in favoritos) {
    ...
    let precio = document.createElement("p");
    precio.classList.add("precio");
    ajuste = String(favoritos[i]["price"]["amount"]).split(".")[1]
      ? String(favoritos[i]["price"]["amount"])
        .split(".")[0]
        .split(/(?=(?:...)*$)/) +
        "." +
        String(favoritos[i]["price"]["amount"]).split(".")[1]
      : String(favoritos[i]["price"]["amount"])
        .split(".")[0]
        .split(/(?=(?:...)*$)/);
...
function deleteSelected() {
  ...
  for (let i = 0; i < ck["length"]; i++) {
    if (ck[i].checked) {
      favoritos = favoritos.filter((func) => {
        return func["id"] != ck[i].parentElement.dataset.item;
      });
    }
  }
  renderFavoritos();
...
function filtrarCategoria() {
  ...
  if (filtrado.length > 0) {
    ...
    alerta.style.top = "130px";
    alerta.innerHTML = "No hay elementos en dicha categoría!, Intenta de nuevo!";
    main.prepend(alerta);
  }
}
